...
def main():
    path_to_model = 'D:\PycharmProjects\myCNN_RNN_attention\data\original_data\GoogleNews-vectors-negative300.bin'
...
def bin2txt(path_to_model, output_file):
    ...
    output = np.array(['', []])
    for word in vocab:
        cur_wordvec = model[word]
        cur_row = np.array([word, cur_wordvec])
        output = np.row_stack((output, cur_row))
    print(output.shape)
